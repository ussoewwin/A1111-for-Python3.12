...
class SD3Tokenizer:
    def __init__(self):
        # transformers 4.30.2 and 4.56.0+ compatibility: handle from_pretrained with proper error handling
        try:
            clip_tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-large-patch14")
        except Exception as e:
            # Fallback: try with trust_remote_code parameter
            try:
                clip_tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-large-patch14", trust_remote_code=True)
            except Exception:
                # If all else fails, try with different parameters
                print(f"Warning: Could not load CLIP tokenizer: {e}")
...
class T5XXLTokenizer(SDTokenizer):
    ...
    def __init__(self):
        ...
        try:
            ...
            # Fallback: try with trust_remote_code parameter
            try:
                tokenizer = T5TokenizerFast.from_pretrained("google/t5-v1_1-xxl", trust_remote_code=True)
            except Exception:
                # If all else fails, try with different parameters
                print(f"Warning: Could not load T5 tokenizer: {e}")
                tokenizer = T5TokenizerFast.from_pretrained("google/t5-base")
        
        super().__init__(pad_with_end=False, tokenizer=tokenizer, has_start_token=False, pad_to_max_length=False, max_length=99999999, min_length=77)
