...
class Sd3ClipLG(sd_hijack_clip.TextConditionalModel):
    def __init__(self, clip_l, clip_g):
        ...
        try:
            ...
            try:
                self.tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-large-patch14", trust_remote_code=True)
            except Exception:
...
class Sd3T5(torch.nn.Module):
    def __init__(self, t5xxl):
        ...
        try:
            ...
            # Fallback: try with trust_remote_code parameter
            try:
                self.tokenizer = T5TokenizerFast.from_pretrained("google/t5-v1_1-xxl", trust_remote_code=True)
            except Exception:
                # If all else fails, try with different parameters
                print(f"Warning: Could not load T5 tokenizer: {e}")
                self.tokenizer = T5TokenizerFast.from_pretrained("google/t5-base")
