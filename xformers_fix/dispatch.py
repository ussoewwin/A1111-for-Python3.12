        ...
        if mqa_or_gqa and inp.query.shape[1] <= 32 and inp.key.shape[1] >= 256:
            ...
            if parallelism_BH > 0 and parallelism_BH < 64:
                ...
                if torch.version.cuda and not isinstance(
                    ...
                    # Only remove FA-2 if FA-3 is not available and we want to prioritize triton_splitk
                    # For now, keep FA-2 in the priority list to ensure it's used when available
