...
def traverse_all_files(curr_path, model_list):
    # Skip silently if path doesn't exist to avoid crashing on stale CLI/UI paths
...
def _normalize_and_expand_paths(raw: str) -> list[str]:
    """Split a raw path string by common separators and normalize/expand each entry."""
    if not raw:
        return []
    candidates = []
    for part in re.split(r"[;,]", str(raw)):
        # remove both double and single quotes that might be kept by Windows shells
...
def update_cn_models():
    cn_models.clear()
    # Collect extra directories from UI option and command line. Accept multiple paths
    # separated by ';' or ','. Normalize and expand env vars.
    ui_raw = shared.opts.data.get("control_net_models_path", None)
    cli_raw = getattr(shared.cmd_opts, 'controlnet_dir', None)

    ui_paths = _normalize_and_expand_paths(ui_raw) if ui_raw else []
    cli_paths = _normalize_and_expand_paths(cli_raw) if cli_raw else []

    # Keep all candidates even if not exists, but report their state
    extra_paths = [*ui_paths, *cli_paths]
    paths = [cn_models_dir, cn_models_dir_old, *extra_paths]
    ...
    # Debug prints to help diagnose missing models
    try:
        debug_states = {p: os.path.exists(p) for p in paths}
        print(f"[ControlNet] Raw UI path: {ui_raw!r}, Raw CLI path: {cli_raw!r}")
        print(f"[ControlNet] Scanned paths: {paths}")
        print(f"[ControlNet] Path exists map: {debug_states}")
        print(f"[ControlNet] Found {len(cn_models)} entries (including 'None').")
    except Exception:
        pass
